  ...
  return (
    <div className="relative w-full py-16 bg-[#0d1117] overflow-hidden">
      <div
        ...
        onMouseEnter={stopAutoScroll}
        onMouseLeave={startAutoScroll}
        className="flex overflow-x-auto space-x-6 px-6 scroll-smooth scrollbar-hide"
      ...
      >
        {[...projects, ...projects].map((project, index) => (
          <div
            ...
            onMouseEnter={() => setHoveredIndex(index)}
            onMouseLeave={() => setHoveredIndex(null)}
            className="w-80 min-w-[20rem] bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl shadow-lg flex-shrink-0 overflow-hidden transition-all duration-300 hover:scale-[1.03]"
          >
            <img
              src={project.imageUrl}
              alt={project.name}
